...
(function ($) {
    ...
    function closeExternal(event) {
        ...
        if ((self.isLocked() && isOverMinWidth(self)) || $(event.target).parents('.' + self.options.classWrapper).size() > 0 || $target.parents('.' + self.options.classWrapper).size() > 0 || $target.hasClass('s-swipe')) {
            return;
        }
    ...
    function initHammer(self) {
        ...
        if ($.fn.hammerScroll) {
            self[hammerScroll] = $('.s-scroller', self.$wrapper).hammerScroll({
                contentWrapperClass: 's-scroller-content',
                eventDelegated:      true,
        ...
        self[swipe] = $('<div id="s-swipe' + self.guid + '" class="s-swipe"></div>').appendTo(self.$element);
    ...
    var Sidebar = function (element, options) {
        ...
        if (Sidebar.POSITION_RIGHT !== this.options.position) {
            ...
            this.$element.addClass('s-right');
        }

        if (this.$element.hasClass('s-right')) {
        ...
        if (null !== this.options.toggleId) {
            ...
            this.$element.addClass('s-togglable');
        }
        ...
        if (this.options.sidebarStickyHeader && $.fn.stickyHeader && !$.fn.hammerScroll) {
            this.stickyHeader = $('.s-scroller', this.$wrapper).stickyHeader().data('st.stickyheader');
        }
     ...
     */
    Sidebar.DEFAULTS = {
        classToggle:         's-toggle',
        classWrapper:        's-wrapper',
        classOpen:           's-open',
        classLocked:         's-locked',
        classForceOpen:      's-force-open',
        classOnDragging:     's-dragging',
        openOnHover:         false,
    ...
    Sidebar.prototype.open = function () {
        ...
        $('[data-s=true]').sidebar('forceClose');
        this.$wrapper.addClass(this.options.classOpen);
    ...
    Sidebar.prototype.toggle = function (event) {
        ...
        if (event) {
            ...
            if ($parents.size() > 0 || $target.hasClass('s-swipe')) {
                return;
            }
        }
